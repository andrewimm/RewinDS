...
public enum Console: Int32 {
    case gba = 0  // REWINDS_CONSOLE_GBA
    case nds = 1  // REWINDS_CONSOLE_NDS

    /// The screens this console presents, top-first.
    public var screenCount: Int { self == .nds ? 2 : 1 }
    /// Native pixel size of each screen.
    public var screenSize: (width: Int, height: Int) { self == .nds ? (256, 192) : (240, 160) }
    public var displayName: String { self == .nds ? "Nintendo DS" : "Game Boy Advance" }
}

/// Why a load failed, mapped from the core's `REWINDS_ERR_*` codes.
public enum LoadError: Int32, Error, CustomStringConvertible {
...
public struct BootImages {
    public var rom: Data?
    public var bios: Data?
    public var bios7: Data?
    public var firmware: Data?

    public init(rom: Data? = nil, bios: Data? = nil, bios7: Data? = nil, firmware: Data? = nil) {
        self.rom = rom
        self.bios = bios
        self.bios7 = bios7
...
public final class EmulatorCore {
    ...
    public init(images: BootImages, forceConsole: Console? = nil) throws {
    ...
    public static func detectConsole(rom: Data) -> Console? {
    ...
    public var frame: UInt64 { rewinds_frame(handle) }
    public var screenCount: Int { Int(rewinds_screen_count(handle)) }
    ...
    public func setLid(closed: Bool) { rewinds_set_lid(handle, closed) }

    public func setAudioMuted(_ muted: Bool) { rewinds_set_audio_muted(handle, muted) }
    ...
    public func withScreen<R>(_ index: Int, _ body: (ScreenBuffer) -> R) -> R? {
    ...
    public var saveDirty: Bool { rewinds_save_dirty(handle) }
    public func clearSaveDirty() { rewinds_clear_save_dirty(handle) }

    /// The cartridge save type's name (e.g. "FLASH1M"), or "none" for a console without
    /// configurable save types (the DS).
    public func saveTypeName() -> String {
    ...
    @discardableResult
    public func setSaveType(name: String) -> Bool {
    ...
    public func linkDeliver(_ frame: Data) {
    ...
    public var linkPending: Bool { rewinds_link_pending(handle) }

    /// Attach/detach a carrier and set this unit's id (0 = parent, 1–3 = child) and the
    /// number of linked units.
    public func setLinkConfig(connected: Bool, id: UInt8, count: UInt8) {
    ...
    public func enableAudio(rate: UInt32, channels: Int) -> AudioConsumer? {
    ...
    public static var coreVersion: UInt32 { rewinds_core_version() }
}
...
public final class AudioConsumer {
    private let handle: OpaquePointer
    ...
    public func read(into out: UnsafeMutablePointer<Float>, capacity: Int) -> Int {
